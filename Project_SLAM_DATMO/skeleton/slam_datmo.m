...
for t = 2:n_timesteps
    ...
    if first_detect<=mov_obj_motion_type+1
        plot_SLAM_DATMO(map, simSteps{t}.truePose, z_t_s, z_t_m, [xm(1,1:t);ym(1,1:t)], sigma_t_1, mu_plot, 0, 0 , color)
        p = p+1;
    else
        %mu_t_m = zeros(3,1);
        %sigma_t_m = zeros(3,3)
        ...
        plot_SLAM_DATMO(map, simSteps{t}.truePose, z_t_s, z_t_m, [xm(1,1:t);ym(1,1:t)], sigma_t_1, mu_plot, sigma_t_m, mu_t_m, color)
    end
    drawnow;
    hold on
end
...
figure()
plot(pose_true(1,:), pose_true(2,:), 'b','LineWidth', 1.5,  'LineStyle','--');
hold on
grid on
scatter(map(1,:), map(2,:), 'k+', 'LineWidth', 1.5)
plot(slam_pose(1,:), slam_pose(2,:), 'LineWidth', 1.5)
hexColor = '#EDB120';  % Hex color code
rgbColor = sscanf(hexColor(2:end), '%2x%2x%2x', [1 3]) / 255;
scatter(mu_t(4:2:end-1,1), mu_t(5:2:end, 1),50, rgbColor,'LineWidth', 1.5)
plot(mov_path_pred(1,p+1:end), mov_path_pred(2,p+1:end), 'LineWidth', 1.5)
plot(xm,ym, 'LineWidth', 1.5, 'LineStyle','--')
legend('True Path', 'True Features', 'SLAM Path', 'SLAM Features', 'DATMO', 'True Move Obj')
...
figure()
subplot(2,1,1)
err_pose_sq = (slam_pose - pose_true).^2;
err_pose_sq(3,251:253) = (slam_pose(3,251:253) + pose_true(3,251:253)).^2;
rmse_err = sqrt(mean(err_pose_sq,1));
plot(rmse_err, 'b', 'LineWidth', 1.2)
grid on

subplot(2,1,2)
err_mov_sq = (mov_path_pred(:,5:end) - [xm(5:end);ym(5:end)]).^2;
rmse_err_mov = sqrt(mean(err_mov_sq,1));
plot(rmse_err_mov, 'r', 'LineWidth', 1.2)
grid on